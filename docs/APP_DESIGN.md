...
**The main thread never stalls on a student's click.** Everything a scene does in response to a click, a slider, a tab change or its first render must finish in well under a frame's worth of time on a low-end laptop — take the number on the development Mac and multiply by four, and treat anything over about 300 ms on the click path as a freeze, because Chrome and Safari offer to kill the tab, and a student reports that as the lab crashing. Simulation that needs longer — a head start so a scorecard has something to say, a measurement sweep, a calibration — is a *debt the world pays down a few tens of milliseconds per frame* inside the frame loop, with a badge over the arena saying what it is doing and how far along it is, and a test asserting that the result is identical to the one-shot run. Never buy the time by lowering the simulation's resolution or shortening the head start; those numbers are tuned and tested. The frame loop clamps the elapsed time it will catch up on (0.05 s) and caps the steps it takes per frame, so a tab returning from the background does not try to simulate the gap. And nothing in the world may grow with the run: traces are ring buffers, event lists are pruned, because a lab runs for a whole session. Lab 3 taught this in 2026-09: its 45-second head start ran synchronously, cost five seconds on an M-series Mac and ten to twenty on a student laptop, and reached students before anyone had measured it.
