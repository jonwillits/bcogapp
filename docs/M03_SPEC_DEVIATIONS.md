...
The shipped version also raises h∞ between rest and threshold, so the "instant" cell is a little more excitable near rest — it fires at small totals the healthy cell ignores. The floor of the input–output curve (totals below zero) is unchanged. The doubled ceiling and the returning spikes are only reachable by the instruments, not by the sliders: at the sliders' limits (total input 200, or 40 µA/cm² injected) the instant cell fires within a few spikes per second of the healthy one, and the reflection shows only for a single pulse. **The handout's Q12 therefore asks about the measured curve**, which is what a student can see: the instant cell fires at small totals the healthy one ignores (29 vs 1 spikes/s at a total of 10), and the right-hand end no longer levels off within the plotted range (215 vs 186 spikes/s at 260). The answer key should say that both ends move for the same reason — more sodium channels are available sooner.
...
- **Inputs are named by absolute side.** x₁ is the left sensor and x₂ the right, in both cells, so the crossed connection is b₂ in the left cell and b₁ in the right; the copy button mirrors rather than copies. The first build numbered inputs relative to the cell (its own side first), which put "same side" and "opposite side" on the panel while the picture drew L and R (Jon, 2026-09-07). **The handout still says "its own side's sensor" in Part 0 and names connections as "first" and "second" in Q6 and Q13; Jon is editing it and asked for it to be left alone.**
- **The biological / artificial toggle is gone, and with it Q27.** It changed every word and no number, which was its point (spec §2.2, the reading's multiple realizability made as a thing you do). Jon's call (2026-09-07): the lab is already challenging and the loss is minor. The old Q28 is now Q27.
