...
**A vehicle needs two cells.** One neuron has one output and cannot steer. The vehicle carries two mirror-image copies on one parameter set — left cell to left wheel from same-side and opposite-side sensors, right cell its mirror — which is Lab 1's 2×2 matrix with the symmetry made explicit. Every control sets both; the Unit and Membrane tabs show the left one. The handout could say this in Part 0.
...
The test asserts what holds: N1 and N2 at most a fifth of healthy, N3 at most three-quarters, N4 below healthy. **N1 in a slow world** is asserted on the *gain* from slowing the world (N1 gains about 2.5 a minute; nobody else more than 0.3), and **N4 on energy per light** at 1.5× rather than 2× (infinite against N1 and N2, about 1.8× against N3). These three count-based tests take a quarter of an hour and live in `neuronWorld.slow.test.ts`, run by `npm run test:slow` rather than by every `npm run test`.
...
8. **Q19 / Part 4.** N4 does not use "a small fraction of the ATP" (§6 above): the resting cost dominates, which is the reading's own point. Reword to "spends a little less, and gets its lights cheaper than the other three".
